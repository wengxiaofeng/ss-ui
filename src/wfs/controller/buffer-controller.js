...
class BufferController extends EventHandler {
  ...
  onBufferAppending(data) {
    // console.log('onBufferAppending data', data)
    if (!this.segments) {
      this.segments = [data];
    } else {
    ...
    if(this.media.err) {
      console.log('before doAppending',this.media.err)
    }
    this.doAppending();
  }
  ...
  onMediaSourceOpen() {
    ...
    this.wfs.trigger(Event.MEDIA_ATTACHED, { media: this.media, channelName: this.channelName, mediaType: this.mediaType, websocketName: this.websocketName,touch:this.touch });
  }
  ...
  createSourceBuffers(tracks) {
    ...
    let mimeType;
    console.log('tracks.mimeType', tracks, tracks.mimeType)
    if (tracks.mimeType === '') {
  ...
  doAppending() {
    ...
    if (Object.keys(sourceBuffer).length) {
      if (this.media.error) {
        this.segments = [];
        console.log(this.media.error,'trying to append although a media error occured, flush segment and abort');
        return;
      }
      if (this.appending) {
        return;
      ...
      if (segments && segments.length) {
        ...
        try {
          ...
          console.log('do doAppending', err)
